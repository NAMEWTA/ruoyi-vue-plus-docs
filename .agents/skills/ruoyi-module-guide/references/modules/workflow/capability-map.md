...
| 其他业务模块现状 | `ruoyi-demo` / `ruoyi-ai` / `ruoyi-job` 源码无 `WorkflowService` / `org.dromara.workflow` 引用。需要向用户发送流程消息时统一提交 `NotificationCommand`，由 Notify 收件箱按 `workflow` 路径归类。 | `ruoyi-vue-plus-namewta/ruoyi-api/src/main/java/org/dromara/notify/api/NotificationApplicationService.java`；`ruoyi-vue-plus-namewta/ruoyi-modules/ruoyi-notify/src/main/java/org/dromara/notify/adapter/inapp/InAppNotificationPortImpl.java` |
