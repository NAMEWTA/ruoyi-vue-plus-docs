...
| 其他业务模块现状 | `ruoyi-demo` / `ruoyi-ai` / `ruoyi-job` 源码无 `WorkflowService` / `org.dromara.workflow` 引用。`ruoyi-system` 仅把 `"workflow"` 当站内信分类。 | `ruoyi-vue-plus-namewta/ruoyi-modules/ruoyi-system/src/main/java/org/dromara/system/service/impl/SysMessageServiceImpl.java`（`CATEGORY_WORKFLOW`）；`ruoyi-vue-plus-namewta/ruoyi-modules/ruoyi-system/src/main/java/org/dromara/system/domain/vo/SysMessageBoxVo.java`（`workflowList`） |
