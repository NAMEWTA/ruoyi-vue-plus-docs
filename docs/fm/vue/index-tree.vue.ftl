...
<#else>
        <el-table-column label="${column.columnLabel}" align="center" prop="${column.javaField}" width="180">
          ...
          <template #default="scope">
            <span>{{ runtime.formatTime(scope.row.${column.javaField}, '{y}-{m}-{d}') }}</span>
          </template>
        </el-table-column>
...
<#else>
        <el-table-column label="${column.columnLabel}" align="center" prop="${column.javaField}Url" width="100">
          ...
          <template #default="scope">
            <component :is="runtime.imagePreview" :src="scope.row.${column.javaField}Url" :width="50" :height="50" />
          </template>
        </el-table-column>
...
<#elseif column.htmlType == "imageUpload">
        <el-form-item label="${column.columnLabel}" prop="${column.javaField}">
          <component :is="runtime.imageUpload" v-model="form.${column.javaField}" />
        </el-form-item>
<#elseif column.htmlType == "fileUpload">
        <el-form-item label="${column.columnLabel}" prop="${column.javaField}">
          <component :is="runtime.fileUpload" v-model="form.${column.javaField}" />
        </el-form-item>
<#elseif column.htmlType == "editor">
        <el-form-item label="${column.columnLabel}">
          <component :is="runtime.editor" v-model="form.${column.javaField}" :min-height="192" />
        </el-form-item>
...
import type {
  ...
  ${BusinessName}Query,
  ${BusinessName}VO
} from '@namewta/domain-${moduleName}/${businessName}';
...
import type { ${BusinessName}WebRuntime } from './runtime';
...
const { runtime } = defineProps<{ runtime: ${BusinessName}WebRuntime }>();

<#if needDict>
const { ${dictsNoSymbol} } = runtime.dicts(${dicts});
</#if>
...
const ${businessName}TableRef = ref<TableInstance>();
