...
read_only_paths: ["<Path>ruoyi-vue-plus-namewta/pom.xml</Path>", "<Path>ruoyi-vue-plus-namewta/ruoyi-admin/pom.xml</Path>", "<Path>ruoyi-vue-plus-namewta/ruoyi-admin/src/test/java/org/dromara/test/notify/**</Path>"]
shared_paths: []
shared_path_owners: []
---
...
- [ ] AC-003 本地可执行部分通过，远程部分明确 pending commit/push。
- [x] 真实浏览器验收发现的 OSS Complete 响应和已完成票据恢复回归通过。
- [ ] 集成测试没有通过 Assumption 静默跳过。
