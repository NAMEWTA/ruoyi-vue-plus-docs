...
| T-06 | backend T-05 result | backend current/main | resolver/cache tests | 2dacefcfb | direct-parent integrated；ACK-based coordinator wiring and fail-closed snapshot path verified locally；two-node Redis E2E pending | 2dacefcfb |
| T-07 | backend T-06 result | backend current/main | Gateway/mock HTTP tests | 2dacefcfb | direct-parent integrated；loopback request/zero-request/302 refusal, endpoint SPI ownership, and typed transport tests pass，application-context HTTP/redirect/SSRF E2E pending | 2dacefcfb |
| T-08 | backend T-07 result | backend current/main | adapter/resilience、port/support、Redisson tests | 2dacefcfb | direct-parent integrated；28 module tests and bounded retry policy pass，Redis+HTTP count/timeout E2E pending | 2dacefcfb |
| T-09 | backend T-08 result | backend current/main | observability/MySQL tests | 2dacefcfb | direct-parent integrated；28 module tests, physical-attempt log capture, sink health, sanitization canaries, and lifecycle guards pass，MySQL aggregate/SysLog persistence E2E pending | 2dacefcfb |
| T-10 | frontend main@587a629a... | frontend current/main | domain test/type/lint | fa3b8b6 | direct-parent integrated；E2E not required | fa3b8b6 |
| T-11 | frontend T-10 result | frontend current/main | web-domain component/type/lint | fa3b8b6 | direct-parent integrated；E2E not required | fa3b8b6 |
| T-12 | frontend T-11 + root SQL | frontend/root current/main | Admin manifest/architecture/build | fa3b8b6 | direct-parent integrated；browser/MySQL/backend E2E pending | fa3b8b6 |
...
4. fresh-init 顺序为基础 10 → third 50 → third baseline DML 60 → third menu/permission 61；生产先部署代码，再经批准开放菜单/角色和 Endpoint。
...
The user authorized implementation commits and direct-parent integration with decision `USER-DECISION:2026-09-04-execute-goal-plan`. T-01 is integrated; T-02 through T-12 have local implementation results and remain in `review` until the release gate; T-13 is the active release gate, all owned by `codex:/root`. Backend implementation is at `2dacefcfb` (including the layered-boundary refactor, port-based adapter dependencies, parameterized response conversion, provider-owned endpoint adapter selection, physical attempt preservation and start/finish logging, hardened provider-origin/header/path metadata validation, runtime snapshot revalidation, safe endpoint header overrides, sanitized JSON persistence, deterministic loopback Gateway coverage, explicit 302 refusal, bounded retry policy, typed `@HttpExchange` transport coverage, outbound log sanitization, sink health signal, expanded key-header redaction, ACK-based `ClusterCacheInvalidationCoordinator` integration, immutable provider-code identity, and deleted-credential reactivation protection); frontend implementation is at `fa3b8b6` with four page registrations, nested credential editing, endpoint header override input, typed table-row handlers, and corrected Admin manifest test doubles. Root menu SQL is integrated at `bed64ec`; DDL remains at `c49f562`. The Skill navigation includes `ruoyi-third` and its layered module guide.

Completed local gates: backend module compile, layered module-mode validation, local-profile crypto/path/sanitizer/origin/loopback Gateway tests (28 passing, including 302 refusal, bounded retry, provider-owned endpoint SPI, typed `@HttpExchange`, physical-attempt logging, sink health, outbound log sanitization, immutable provider-code identity, and deleted-credential reactivation guards), and the latest full backend `./mvnw.cmd -P local -q test` command (exit 0; the `ruoyi-third` module contributes 28 local-profile tests with 0 failures/errors). Frontend domain/web-domain/Admin typecheck, lint, package build/test gates, architecture check, and Admin manifest registry tests also pass after fixing the table-row handler inference and service mock contract. Runtime probing found reachable MySQL/Redis services and confirmed that a current classpath launch loads `ruoyi-third` and opens HTTP `18081`; a read-only JDBC probe confirmed MySQL `8.4.9` but found no `third_*` tables in the existing `ry-namewta` database. The loopback Gateway tests prove constrained delivery, endpoint adapter ownership, physical-attempt capture, typed transport, and pre-send zero-request rejection without external credentials. The launch exits in the existing system OSS runner when the shared Redis cache-invalidation acknowledgement times out for a second instance. The legacy `18080` JAR predates this module and returned `404` for `/third/provider/list`. Docker CLI, isolated fresh-init/counting provider infrastructure, application-context HTTP status/redirect/retry, production SysLog persistence, and browser permission E2E remain unavailable or unapproved; they remain explicit release prerequisites and are recorded as pending in T-13 evidence. The change remains `in_progress` until those required external gates are run by the release environment.

The release bundle gate was subsequently verified from an isolated export of backend `2dacefcfb` at `D:\Document\code\namewta-release-verify-20260905-v2`: clean `package -Pbundle-full -DskipTests` and `clean package -Pbundle-core "-Dmaven.test.skip=true"` both completed all 47 reactor projects with `ruoyi-admin` repackage success. Direct `BOOT-INF/lib` inspection found `ruoyi-third-6.0.0.jar` in both packages; job/ai/demo/workflow artifacts were present only in full. The isolated result is authoritative for bundle composition because the original workspace contains a running legacy `ruoyi-admin` process that locks `ruoyi-admin/target/ruoyi-admin.jar`. Verified copies are `D:\Document\code\namewta-full-admin-20260905-v4.jar` and `D:\Document\code\namewta-core-admin-20260905-v4.jar`.

Latest execution checkpoint: backend `2dacefcfb`; frontend `fa3b8b6`; DDL `c49f562`; menu DML `bed64ec`. Isolated full/core packaging and `BOOT-INF/lib` composition checks pass, and the release-artifacts static suite reports 31 passing tests for the ordered 10/20/30/40/50/60/61 baseline. SpecDev `tickets` and `implement` validators remain at 0 errors / 0 warnings.

#### Follow-up verification (2026-09-05)

Backend `2dacefcfb` was revalidated after endpoint SPI, typed transport, physical-attempt logging, sink-health, expanded key-header redaction, ACK-based cache invalidation, immutable provider-code identity, and deleted-credential reactivation protection additions: `./mvnw.cmd -P local -q test` exited 0, `ruoyi-third` reports 28 local-profile tests with 0 failures/errors, and layered module validation passes for 70 Java files. Frontend `fa3b8b6` was revalidated with domain/web-domain/Admin typecheck, lint, targeted tests, and architecture checks; Admin reports 40 tests with 0 failures. The isolated full/core package evidence was rebuilt from the backend checkpoint and is recorded below. The release-artifacts static suite reports 31 passing tests. The SpecDev `tickets` and `implement` validators remain at 0 errors / 0 warnings. These checks supersede the earlier `8d9d03238`/`1fa13dc` package/test checkpoint while preserving the pending external release gates below.
...
- 最新检查点：backend `2dacefcfb`，frontend `fa3b8b6`，DDL `c49f562`，菜单 DML `bed64ec`；tickets/implement 校验均为 0 error/0 warning，release-artifacts 静态套件 31 项通过。
- 外部 Gate：MySQL 8.4 fresh-init、隔离 Redis 多实例失效、确定性本地 HTTP 应用上下文状态/重定向/重试、生产 SysLog 持久化和浏览器权限 E2E 仍需 release 环境执行；full/core 完整构建已在隔离 backend `2dacefcfb` 导出目录通过，并完成 `BOOT-INF/lib` 组合核对。当前只读探测确认 MySQL 8.4.9 但 schema 尚未初始化，并确认旧 JAR 404 和新类路径实例被共享 Redis invalidation acknowledgement timeout 阻断，均记录为 pending。
