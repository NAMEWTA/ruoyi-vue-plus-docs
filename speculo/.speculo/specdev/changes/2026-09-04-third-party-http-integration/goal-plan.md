...
| T-07 | backend T-06 result | backend current/main | Gateway/mock HTTP tests | 650a5bd98 | direct-parent integrated；loopback request/zero-request/302 refusal, endpoint SPI ownership, and typed transport tests pass，application-context HTTP/redirect/SSRF E2E pending | 650a5bd98 |
| T-08 | backend T-07 result | backend current/main | adapter/resilience、port/support、Redisson tests | 650a5bd98 | direct-parent integrated；26 module tests and bounded retry policy pass，Redis+HTTP count/timeout E2E pending | 650a5bd98 |
| T-09 | backend T-08 result | backend current/main | observability/MySQL tests | 650a5bd98 | direct-parent integrated；26 module tests, physical-attempt log capture, sink health, and sanitization canaries pass，MySQL aggregate/SysLog persistence E2E pending | 650a5bd98 |
...
The release bundle gate was subsequently verified from an isolated export of backend `650a5bd98` at `D:\Document\code\namewta-release-verify-20260905-v1`: clean `package -Pbundle-full -DskipTests` and `clean package -Pbundle-core "-Dmaven.test.skip=true"` both completed all 47 reactor projects with `ruoyi-admin` repackage success. Direct `BOOT-INF/lib` inspection found `ruoyi-third-6.0.0.jar` in both packages; job/ai/demo/workflow artifacts were present only in full. The isolated result is authoritative for bundle composition because the original workspace contains a running legacy `ruoyi-admin` process that locks `ruoyi-admin/target/ruoyi-admin.jar`. Verified copies are `D:\Document\code\namewta-full-admin-20260905-v2.jar` and `D:\Document\code\namewta-core-admin-20260905-v2.jar`.

Latest execution checkpoint: backend `650a5bd98`; frontend `1fa13dc`; DDL `c49f562`; menu DML `bed64ec`. Isolated full/core packaging and `BOOT-INF/lib` composition checks pass. SpecDev `tickets` and `implement` validators remain at 0 errors / 0 warnings.

#### Follow-up verification (2026-09-05)

Backend `650a5bd98` was revalidated after endpoint SPI, typed transport, physical-attempt logging, sink-health, and expanded key-header redaction additions: `./mvnw.cmd -P local -q test` exited 0, `ruoyi-third` reports 26 local-profile tests with 0 failures/errors, and layered module validation passes for 70 Java files. The isolated full/core package evidence was rebuilt from this checkpoint and is recorded below. The SpecDev `tickets` and `implement` validators remain at 0 errors / 0 warnings. These checks supersede the earlier `5401cbc08` package/test checkpoint while preserving the pending external release gates below.
...
- 最新检查点：backend `650a5bd98`，frontend `1fa13dc`，DDL `c49f562`，菜单 DML `bed64ec`；tickets/implement 校验均为 0 error/0 warning。
- 外部 Gate：MySQL 8.4 fresh-init、隔离 Redis 多实例失效、确定性本地 HTTP 应用上下文状态/重定向/重试、生产 SysLog 持久化和浏览器权限 E2E 仍需 release 环境执行；full/core 完整构建已在隔离 backend `650a5bd98` 导出目录通过，并完成 `BOOT-INF/lib` 组合核对。当前只读探测确认 MySQL 8.4.9 但 schema 尚未初始化，并确认旧 JAR 404 和新类路径实例被共享 Redis invalidation acknowledgement timeout 阻断，均记录为 pending。
