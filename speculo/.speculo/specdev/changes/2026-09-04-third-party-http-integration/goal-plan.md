...
| T-12 | frontend T-11 + root SQL | frontend/root current/main | Admin manifest/architecture/build | d45a480 | direct-parent integrated；browser/MySQL/backend E2E pending | d45a480 |
...
Latest execution checkpoint: backend `2dacefcfb`; frontend `d45a480`; DDL `c49f562`; menu DML `bed64ec`. Isolated full/core packaging and `BOOT-INF/lib` composition checks pass, and the release-artifacts static suite reports 31 passing tests for the ordered 10/20/30/40/50/60/61 baseline. SpecDev `tickets` and `implement` validators remain at 0 errors / 0 warnings.

#### Follow-up verification (2026-09-05)

Backend `2dacefcfb` was revalidated after endpoint SPI, typed transport, physical-attempt logging, sink-health, expanded key-header redaction, ACK-based cache invalidation, immutable provider-code identity, and deleted-credential reactivation protection additions: `./mvnw.cmd -P local -q test` exited 0, `ruoyi-third` reports 28 local-profile tests with 0 failures/errors, and layered module validation passes for 70 Java files. Frontend `d45a480` was revalidated with full workspace `pnpm typecheck`, `pnpm lint`, `pnpm test`, and `pnpm build`, plus domain/web-domain/Admin targeted gates; all completed successfully, and Admin reports 40 tests with 0 failures. The architecture test caught a stale six-domain registry and now passes with the `third -> ruoyi-third` mapping (101/101 tests). The isolated full/core package evidence was rebuilt from the backend checkpoint and is recorded below. The release-artifacts static suite reports 31 passing tests. The SpecDev `tickets` and `implement` validators remain at 0 errors / 0 warnings. These checks supersede the earlier `8d9d03238`/`1fa13dc` package/test checkpoint while preserving the pending external release gates below.
...
- 最新检查点：backend `2dacefcfb`，frontend `d45a480`，DDL `c49f562`，菜单 DML `bed64ec`；tickets/implement 校验均为 0 error/0 warning，release-artifacts 静态套件 31 项通过。
