...
Verified locally: backend `ruoyi-third` compile, `validate-module-mode.mjs ... --mode layered`, and local-profile crypto/path/sanitizer/origin/loopback Gateway tests (28 passing, including redirect refusal, bounded retry, provider-owned endpoint adapter validation, typed `@HttpExchange`, physical-attempt log capture, sink health, outbound log sanitization, provider key-header redaction, immutable provider-code identity, and deleted-credential reactivation rejection); frontend domain, web-domain, and admin typecheck/lint/build/test; full workspace typecheck/lint/test/build; architecture check (101/101, including `third -> ruoyi-third`); and admin production build. The backend module now follows `controller/admin -> usecase/impl -> service -> dao -> mapper -> XML`, with runtime ports/adapters/support boundaries, and `ruoyi-module-guide` registers the module. The latest backend result is `2dacefcfb`; frontend result is `d45a480`; root menu SQL is integrated at `bed64ec`.
