...
Verified locally: backend `ruoyi-third` compile, `validate-module-mode.mjs ... --mode layered`, and local-profile crypto/path/sanitizer/origin/loopback Gateway tests (26 passing, including redirect refusal, bounded retry, provider-owned endpoint adapter validation, typed `@HttpExchange`, physical-attempt log capture, sink health, outbound log sanitization, and provider key-header redaction); frontend domain, web-domain, and admin typecheck/lint/build/test; architecture check; admin production build. The backend module now follows `controller/admin -> usecase/impl -> service -> dao -> mapper -> XML`, with runtime ports/adapters/support boundaries, and `ruoyi-module-guide` registers the module. The latest backend result is `650a5bd98`; frontend result is `1fa13dc`; root menu SQL is integrated at `bed64ec`.

The full/core bundle gate is also verified from an isolated export of backend `650a5bd98` at `D:\Document\code\namewta-release-verify-20260905-v1`: both clean package commands completed all 47 reactor projects, and direct `BOOT-INF/lib` inspection found `ruoyi-third-6.0.0.jar` in both packages. Optional job/ai/demo/workflow artifacts are present in full and absent from core. Verified copies are `D:\Document\code\namewta-full-admin-20260905-v2.jar` and `D:\Document\code\namewta-core-admin-20260905-v2.jar`. The isolated export was required because the original workspace's running legacy `ruoyi-admin` process locks the target JAR.
